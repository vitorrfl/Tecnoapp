...
(function () {
  ...
  const BENCH_ROTULOS = {
    ...
    cpu_multi:  'Processador (todos)',
    memoria:    'Memória',
    disco:      'Disco'
  };

  function benchDetalhe(chave, v) {
    if (chave === 'cpu_single' || chave === 'cpu_multi') return v.tempo + 's';
    if (chave === 'memoria') return v.gbs + ' GB/s';
    if (chave === 'disco') return v.leitura_mbs + ' MB/s leitura · '
                                + v.escrita_mbs + ' MB/s escrita';
    return '';
  }

  window.runBench = function () {
    if (!window.bridge || !window.bridge.runBenchmark) return;
    const btn = document.getElementById('bench-btn');
    if (btn) {
      btn.disabled = true; btn.style.opacity = '.5'; btn.style.cursor = 'wait';
      btn.textContent = 'MEDINDO...';
    }
    document.getElementById('bench-resultado').style.display = 'none';
    setBind('bench_dica', 'Não use o PC durante a medição — isso altera o resultado.');
    window.bridge.runBenchmark(true);
  };

  window.onBenchStep = function (texto, pct) {
    setBind('bench_etapa', texto);
    const b = document.getElementById('bench-bar');
    if (b) b.style.width = pct + '%';
  };

  window.onBenchFailed = function (erro) {
    const btn = document.getElementById('bench-btn');
    if (btn) {
    ...
    setBind('bench_etapa', '');
    setBind('bench_dica', 'Não foi possível concluir a medição. (' + erro + ')');
  };

  window.onBenchDone = function (r) {
    const btn = document.getElementById('bench-btn');
    if (btn) {
      btn.disabled = false; btn.style.opacity = '1'; btn.style.cursor = 'pointer';
      btn.textContent = 'MEDIR NOVAMENTE';
    }
    if (!r) return;

    setBind('bench_total', String(r.total || 0));
    setBind('bench_etapa', '');
    setBind('bench_quando', r.gamer_ativo
      ? 'Medido com o Modo Gamer ATIVO'
      : 'Medido com o Modo Gamer desligado');
    setBind('bench_dica', 'Rode de novo com o Modo Gamer ligado para comparar.');

    // Linhas de resultado
    const box = document.getElementById('bench-linhas');
    let html = '';
    Object.keys(BENCH_ROTULOS).forEach(function (k) {
      const v = r[k];
      if (!v) return;
      html += '<div style="display:flex;align-items:baseline;gap:12px;padding:7px 0;'
        + 'border-bottom:1px solid var(--border-subtle)">'
        + '<span style="flex:1;font-size:11px;color:var(--fg-primary)">'
        + BENCH_ROTULOS[k] + '</span>'
        + '<span class="mono" style="font-size:10px;color:var(--fg-muted)">'
        + benchDetalhe(k, v) + '</span>'
        + '<span class="mono" style="font-size:12px;color:var(--cyan);font-weight:700;'
        + 'min-width:70px;text-align:right">' + v.pontos + ' pts</span>'
        + '</div>';
    });

    if (r.gpu) {
      html += '<div style="display:flex;align-items:baseline;gap:12px;padding:7px 0">'
        + '<span style="flex:1;font-size:11px;color:var(--fg-primary)">Placa de vídeo</span>'
        + '<span class="mono" style="font-size:10px;color:var(--fg-muted)">'
        + 'em uso durante o teste</span>'
        + '<span class="mono" style="font-size:12px;color:var(--fg-secondary);'
        + 'min-width:70px;text-align:right">' + r.gpu.uso_pct + '%</span></div>';
    }

    // O resultado depende do que a maquina esta fazendo — dizer isso evita
    // que o usuario compare medicoes feitas em condicoes diferentes.
    if (r.contexto && r.contexto.processos) {
      html += '<div style="font-size:10px;color:var(--fg-muted);margin-top:10px;'
        + 'line-height:1.5">Durante a medição: CPU em ' + r.contexto.cpu_pct
        + '%, memória em ' + r.contexto.ram_pct + '%, '
        + r.contexto.processos + ' processos abertos.</div>';
    }
    if (box) box.innerHTML = html;
    document.getElementById('bench-resultado').style.display = 'block';

    // Comparacao com a medicao anterior
    const cmp = document.getElementById('bench-comparacao');
    const ant = r.anterior;
    if (cmp && ant && ant.total) {
      const dif = (r.total || 0) - ant.total;
      const pct = ant.total ? Math.round(dif / ant.total * 100) : 0;
      const cor = dif > 0 ? 'var(--state-on)' : (dif < 0 ? '#e8a33d' : 'var(--fg-muted)');
      const sinal = dif > 0 ? '+' : '';

      let txt = '<div style="font-size:15px;font-weight:700;color:' + cor + '">'
        + sinal + dif + ' pontos (' + sinal + pct + '%)</div>'
        + '<div style="margin-top:6px">Anterior: ' + ant.total + ' pts'
        + (ant.quando ? ' · ' + ant.quando : '')
        + (ant.gamer_ativo ? ' · Modo Gamer ativo' : '') + '</div>';

      // A comparacao so significa algo se o Modo Gamer mudou entre as duas
      if (!!ant.gamer_ativo !== !!r.gamer_ativo) {
        txt += '<div style="margin-top:8px;color:var(--cyan)">'
          + 'Esta é a diferença que o Modo Gamer fez nesta máquina.</div>';
      } else {
        txt += '<div style="margin-top:8px;color:var(--fg-muted)">'
          + 'As duas medições foram feitas na mesma condição — variações pequenas '
          + 'são normais entre execuções.</div>';
      }
      document.getElementById('bench-delta').innerHTML = txt;
