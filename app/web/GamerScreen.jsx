...
function GamerScreen() {
  ...
  function simulate(action) {
    if (window.bridge) {
      if (action === 'activate'   && window.bridge.runGamerActivate)   window.bridge.runGamerActivate();
      if (action === 'deactivate' && window.bridge.runGamerDeactivate) window.bridge.runGamerDeactivate();
    }
    setRunning(true);
    const lines = [];
